...
for t in ['train','test']:
    ...
    for R_clustering,f in [(0.3,basepath+'/npyfilesregression/subjet_oriented_'),
        ...
        ## In[]:
        #random permutation if needed
        if t="train":
            flush = np.random.permutation(len(X))
        else :
            flush = np.arange(len(X_))
        ...
        np.save(f+"seqpt_with_id_"+t+".npy", np.array([X_, y_]))
        ...
        np.save(f+"seqpt_reversed_with_id_"+t+".npy", np.array([X_, y_]))
